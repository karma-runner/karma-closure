...
var DependencyResolver = function(logger) {
  ...
  /** @constructor */
  function CircularDependencyTracker() {
    this.visitedFileMap = Object.create(null);
    this.stack = [];
    this.hasError = false;
  }

  CircularDependencyTracker.prototype.push = function (file) {
    if (this.hasError) return false;

    this.stack.push(file);
    if (this.visitedFileMap[file]) {
      log.error('CIRCULAR DEPENDENCY:', [].concat(this.stack).reverse().join(' <- '))
      this.hasError = true;
      return false;
    }
    this.visitedFileMap[file] = true;
    return true;
  };

  CircularDependencyTracker.prototype.pop = function (file) {
    this.stack.pop();
  };

  var resolveFile = function(filepath, files, alreadyResolvedMap, circDepTracker) {
    if (!fileMap[filepath]) {
    ...
    if (!circDepTracker.push(filepath)) {
      return;
    }
    ...
    fileMap[filepath].requires.forEach(function(dep) {
      if (!alreadyResolvedMap[dep]) {
        if (provideMap[dep]) {
          resolveFile(provideMap[dep], files, alreadyResolvedMap, circDepTracker);
        } else {
  ...
  this.resolveFiles = function(files) {
    ...
    files.forEach(function(file) {
      resolveFile(file, resolvedFiles, alreadyResolvedMap, new CircularDependencyTracker());
    });
